...
public class SearchRoundTripFlightTest extends base {
	...
	public void selectDropdown() throws InterruptedException {
		...
		Select dropdownCurrency = new Select(Currency);
		 //w.until(ExpectedConditions.elementSelectionStateToBe(By.id("ctl00_mainContent_DropDownListCurrency"),
		// true));
	...
	public void autoSuggestDropdown() {
	  ...
	  @Test(dependsOnMethods = "autoSuggestDropdown") public void selectCheckBox()
	  { FlightPageObjects fl = new FlightPageObjects(driver); if
	  (fl.checkStudent().isSelected()) { int checkCount =
	  driver.findElements(By.cssSelector("input[type*='checkbox']")).size();
	  Assert.assertEquals(checkCount, 6); } else { fl.checkStudent().click();
